...
--To modify the value(s) in existing records in a table
UPDATE cars
SET color = 'red'
WHERE brand = 'Volvo';
--be careful without the WHERE clause, ALL records will be updated.
...
--Continue the alter, - change the data type of column
--ALTER column
ALTER TABLE cars
    ALTER COLUMN year TYPE VARCHAR(4);
---NOTE: Note: Some data types cannot be converted if the column has value.
...
--To delete existing records(rows) in a table.
--NOTE: BE CAREFUL! if you will not specify by where all records(rows) would be deleted!
DELETE
FROM cars
WHERE brand = 'Volvo';
-- these two do the same: DELETE FROM cars; == TRUNCATE TABLE cars;

--To drop an existing table in a database.
DROP TABLE cars;
--after this displaying the table will occur an error because the table does no longer exist
...
-- LIMIT - to limit the maximum number of records to return.
SELECT *
FROM customers
LIMIT 10;
...
FROM customers
LIMIT 20 OFFSET 40;
--IT WILL GIVE US 20 RECORDS FROM 40(NOT INCLUDE 40) !!
